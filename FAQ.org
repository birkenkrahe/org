...
for presentation. But it will be no shorter than 10-15 minutes
followed by a short discussion (5 min).
...
* How can I get faster at writing code (without AI)?

A student in my CS I (introduction to programming with Python) class
asked this question with reference to [[https://github.com/birkenkrahe/cosc_1210_fall_26/blob/main/assignments/5_savings_growth_bonus.org][this bonus exercise]] and [[https://github.com/birkenkrahe/cosc_1210_fall_26/blob/main/assignments/5_enzyme_kinetics_bonus.org][this
one]]: she felt that she should have completed them faster.

A few points to note about that:

1) After only 1 month of coding, speed is not important. What is
   important is diligence (and joy).

2) The assignments were labeled "medium" and "hard". The difficulty
   came from several sources: They were very verbose, so you had to
   spend a lot of time reading what was asked of you; and they
   required independent research (the formulas were not given).

3) As you mature as a programmer, three things will happen:

   1. You'll spend most of the time (hopefully) planning your
      approach, picking a language, mastering the infrastructure (the
      support for your programs), etc.

   2. You'll spend less time doing the actual coding (in fact, in an
      AI-assisted world, you'll do less and less of that over time).

   3. You'll spend much of your time debugging, fixing mistakes, and
      making your already working programs faster, or better in other
      ways (more usable, more maintainable, more flexible).

   4. Speed is a function of many factors: skill, the problem, the
      data (input), your machine (device), programming language(s),
      and computing environment - and also who gave you the problem.

   At the start, this is just a lot to think about. You'll get faster.

   Recommendation: develop speed by solving problems at
   open.kattis.com, and if you spend too much time on a problem, ask
   AI to give you a simpler problem. Don't ask for the solution, ask
   for a critique of your solution first.

* What's special in Python about functions?

Python functions do not declare an explicit ~return~ type.

#+begin_src python :results output :exports both :session *Python* :python python3
  def add(spam,ham):
      return spam + ham

  print(add(2,3))    # returns integer
  print(add(2.5,3))  # returns float
#+end_src
...
In other languages, like C/C++ or Java, a function needs to declare
what data type it returns, e.g.

#+begin_src java :dir src
  // Java: this function returns nothing = void
...
#+begin_src C++ :main yes :includes <iostream> <cstdlib> <string> <fstream> <vector> :namespaces std :results output :exports both :noweb yes
  ...
  // C++: this function returns an integer (0 = program ran OK)
  int main()
  {
    std::cout << "Hello, world!" << std::endl;

    return 0; // return 0 if the program ran OK
  }
#+end_src
...
In R you also don't need to declare a ~return~ type (you don't even need
the keyword ~return~: a function returns its last value):
...
* When should you write a docstring in Python?

Another student question: when should you write a docstring? 

Docstrings in Python are strings that are inserted after the function
declaration to say what the function does, and specify input and
output values, and anything else worth knowing when you use the
function. Example:
...
*Answer:* write a docstring when your function is finished - when it
compiles and works as you want it to. The docstring is for others or
for yourself when you use it - so it needs to be usable first.

* Should you design programs top-down or bottom-up?

The student asked: when I have a problem that consists of parts,
should I start with the big parts and work my way down to the small
parts, or the other way around?

*Answer:* Both work, and good programmers use both.

- *Top-down* means you start with the whole: what goes in, what comes
  out, and which parts the solution needs. You write the big function
  first and let it call parts that you haven't written yet. This works
  when you understand the problem well enough to name the parts.

- *Bottom-up* means you start with a small piece that you can test
  right away, then build bigger pieces from it. This works when you
  don't yet know what the parts should be: tinker with small pieces of
  code that don't do much, and let them show you the shape of the
  problem.

Example: the ~receipt~ practice [[https://github.com/birkenkrahe/cosc_1210_fall_26/blob/main/org/5_functions.org#practice-receipt][from the functions lecture]]. Top-down,
you start with ~receipt(item, price, qty)~: it prints a line of
dashes, the item line, the total, and another line of dashes - so it
needs a part called ~rule()~, which you write next. Bottom-up, you
write and test ~rule()~ first, then build ~receipt~ out of it.

A good rule of thumb: *plan top-down, build and test bottom-up.*
Sketch the parts first so you know where you are going. Then write
them one at a time, smallest first, and test each before you move on.

Whichever way you start, *refactoring* improves the design afterwards:
you reorganize working code without changing what it does.

* In Jupyter notebooks, do you always have to write text?

Jupyter notebooks are for "literate programming". They can contain
layout (like headlines, fonts, links, images), source code, and
output - all to make it easier for humans to read programs.

You don't always have to write (a lot of) text outside of the source
code, unless I ask for it explicitly.

The level of explanations depend on your process and on your needs:
- You may want to keep the notebook for later use or to give it to
  someone, e.g. in your GitHub portfolio, or to publish it as a report
  or paper.
- You may have a lot of tangential, perhaps useful thoughts that you
  want to log (but they don't belong in the source code). 
- You may write a notebook (in a project) with other people, in which
  case you can use it to discuss your work in the notebook.

For simple problems, like [[https://github.com/birkenkrahe/cosc_1210_fall_26/blob/main/assignments/5_certificate_printer.org][certificate printer]], where every simple
function has to have its own docstring, it may seem overkill to write
any text at all apart from the task chapter headings.

However, the case in favor of writing at least a little text here,
too, is that you're developing a *habit* - you need to write notebooks
for small problems at first so that later, when the problems get
larger, you have a habit that you can fall back on without having to
think or force yourself.

That's why it's important to train yourself to "think out loud" when
you write programs - and put those thoughts, without too much
filtering, into the notebook.
