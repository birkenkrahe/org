...
#+begin_src R :session *R* :results output :exports both :noweb yes
  ...
  solar <- read.csv("data/solar.csv", header = TRUE)
  lunar <- read.csv("data/lunar.csv", header = TRUE)

  ## correct header names for better display
  selection <- c(2,3,7,12,13,16)
  names(lunar)[selection] <- c('date','time','type','lat','lon','duration')
  head(lunar)[selection]
#+end_src

#+RESULTS:
:                date     time type lat  lon duration
